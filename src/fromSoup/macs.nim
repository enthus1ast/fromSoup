...
when isMainModule and false:
  ...
  proc parseMACAddressAI(text: string): seq[string] {.exportc.} =
    var macAddresses = newSeq[string]()
    let pattern = re"[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}"
    let matches = re.findall(text, pattern)
    for match in matches:
      macAddresses.add(match)
    return macAddresses

  echo parseMACAddressAI("asdfA0:AF:BD:9A:31:B5adf")

  import benchy
  timeIt "me":
    for mac in parseMacs "asdfA0:AF:BD:9A:31:B5adf".repeat(1000):
      keep mac
  timeIt "ai":
    for ip in parseMACAddressAI("asdfA0:AF:BD:9A:31:B5adf".repeat(1000)):
      keep ip